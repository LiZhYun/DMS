...
from pathlib import Path
import torch
from configs.config_sc2 import get_config
...
def parse_args(args, parser):
    parser.add_argument('--map_name', type=str, default='MMM2',help="Which smac map to run on")
...
def main(args):
    ...
    long_short_clip = ""
    if all_args.long_short_clip:
        long_short_clip = "long_short_clip"
    else:
        long_short_clip = "no_clip"

    run_dir = Path(os.path.split(os.path.dirname(os.path.abspath(__file__)))[
                       0] + "/results") / all_args.env_name / all_args.map_name / all_args.algorithm_name / all_args.experiment_name / long_short_clip / str(all_args.seed)
