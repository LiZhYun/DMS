...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        for i in range(self.n_agents):
            ...
            # all_feats, [n_allies, n_ally_feats], [n_enemies, n_enemy_feats], [1, move_feats], [1, own_feats+agent_id_feats+timestep_feats]
    ...
    def _launch(self):
        ...
        if map_info.pathing_grid.bits_per_pixel == 1:
            ...
            self.pathing_grid = np.invert(np.flip(np.transpose(np.array(  # 按位取否
    ...
    def reset(self):
        ...
        available_actions = []
        for i in range(self.n_agents):
            available_actions.append(self.get_avail_agent_actions(i))  # e.g. [0] * (6 + 敌军数量)

        if self.debug:
            logging.debug("Started Episode {}"
                          .format(self._episode_count).center(60, "*"))

        if self.use_state_agent: # false
        ...
        # move_feats: [4]  
        # enemy_feats: [敌军数量, 是否可攻击到 + 相对可视范围的距离 + 相对x + 相对y + 生命值比例 + 护盾值比例 + 敌军类型]
        # ally_feats: [友军数量 - 1, 是否可以看到 + 相对可视范围的距离 + 相对x + 相对y + 生命值比例 + 护盾值比例 + 友军类型 + 上次动作]
        # own_feats: [1(自己看得到自己) + 0(距离为0) + 0(相对x为0) + 0(相对y为0) + 生命值比例 + 护盾值比例 + 自己类型 + 上次动作]
        #local_obs = self.get_obs() # [友军数量, []]
        local_obs_act = self.get_obs()
        all_last_actions = local_obs_act[1]
        local_obs = local_obs_act[0]
        ...
        return local_obs, global_state, available_actions, all_last_actions
    ...
    def step(self, actions):
        ...
        self.last_action = np.eye(self.n_actions)[np.array(actions_int)] # e.g. [[0. 1. 0. 0. 0.] [0. 0. 1. 0. 0.]]
        ...
        try:
            ...
            return local_obs, global_state, [[0]]*self.n_agents, dones, infos, available_actions, all_last_actions
        ...
        return local_obs, global_state, rewards, dones, infos, available_actions, all_last_actions

    def get_agent_action(self, a_id, action): # move[direction], attack[enemy_id], stop, no-op
    ...
    def reward_battle(self): # 伤害(hit-point damage)，击杀(enemy kill)，获胜(winning)。也可以设置成稀疏奖励，输赢反馈+1/-1。
    ...
    def get_obs_agent(self, agent_id):
        # 观察到的特征向量包含友方和敌方的属性信息：
        # [distance,relative x, relative y, health, shield, unit type]。
        # 智能体可以访问视野内友军单位的最后一个动作。
        # 周围的地形特征；特别是，在固定半径处的八个点的值表示高度和可行走性。
        ...
        ally_last_actions = np.zeros((self.n_agents, self.action_dim), dtype=np.float32)
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
                ...
                if (dist < sight_range and al_unit.health > 0):  # visible and alive
                    ...
                    if self.obs_last_action:
                        ...
                        for action_dim_idx in range(self.action_dim):
                            if np.any(self.last_action[al_id] == 1):
                                ally_last_actions[al_id, action_dim_idx] = np.argwhere(self.last_action[al_id] == 1)[0][0]
                            else:
                                ally_last_actions[al_id, action_dim_idx] = 0.
            ...
            if self.obs_last_action:
                ...
                for action_dim_idx in range(self.action_dim):
                    if np.any(self.last_action[agent_id] == 1):
                        ally_last_actions[agent_id, action_dim_idx] = np.argwhere(self.last_action[agent_id] == 1)[0][0]
                    else:
                        ally_last_actions[agent_id, action_dim_idx] = 0.
                #ally_last_actions[agent_id] = np.argwhere(self.last_action[agent_id] == 1) 
        ...
        return [agent_obs, ally_last_actions]
    ...
    def get_obs(self):
        ...
        during decentralised execution.
        """
        agents_obs = [self.get_obs_agent(i)[0] for i in range(self.n_agents)] # e.g. [友军数量, 64]
        agents_last_actions = [self.get_obs_agent(i)[1] for i in range(self.n_agents)] # e.g. [agent_num, agent_num, 1]
        return [agents_obs, agents_last_actions]
    ...
    def get_state(self, agent_id=-1):
        ...
        if self.obs_instead_of_state:
            obs_concat = np.concatenate(self.get_obs()[0], axis=0).astype(np.float32)
            return obs_concat
    ...
    def get_state_own_feats_size(self):
        """Returns the size of the vector containing the agents' own features.
        """
        own_feats = 4 + self.unit_type_bits  # distance, relative x, relative y, shield, and unit_type 
        if self.obs_own_health:  # health
            own_feats += 1 + self.shield_bits_ally

        if self.obs_last_action: #  last actions 
            own_feats += self.n_actions
    ...
    def get_obs_move_feats_size(self):  # n_actions_move + height and walkability
    ...
    def get_state_move_feats_size(self):  # n_actions_move + height and walkability
        """Returns the size of the vector containing the agents's movement-related features."""
        move_feats = self.n_actions_move
        if self.state_pathing_grid:
            move_feats += self.n_obs_pathing
    ...
    def get_obs_size(self):
        ...
        # 4 + self.unit_type_bits + 1 + self.shield_bits_ally + self.n_actions
        own_feats = self.get_obs_own_feats_size() # distance, relative x, relative y, health, shield, and unit_type + last actions
        # self.n_actions_move 4
        move_feats = self.get_obs_move_feats_size() # n_actions_move

        n_enemies, n_enemy_feats = self.get_obs_enemy_feats_size() # [敌军数量,  4 + self.unit_type_bits + 1 + self.shield_bits_enemy]
        # [友军数量 - 1, 4 + self.unit_type_bits + 1 + self.shield_bits_ally + self.n_actions]
        n_allies, n_ally_feats = self.get_obs_ally_feats_size()
        self.n_ally_feats = n_ally_feats
