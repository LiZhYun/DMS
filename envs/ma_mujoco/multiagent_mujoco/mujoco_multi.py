...
class MujocoMulti(MultiAgentEnv):
    ...
    def __init__(self, batch_size=None, **kwargs):
        ...
        # COMPATIBILITY
        self.n = self.n_agents # 4
        # self.observation_space = [Box(low=np.array([-10]*self.n_agents), high=np.array([10]*self.n_agents)) for _ in range(self.n_agents)]
        self.observation_space = [Box(low=-10, high=10, shape=(self.obs_size + self.n_agents,)) for _ in range(self.n_agents)] # [Box(115,), Box(115,), Box(115,), Box(115,)]
        self.share_observation_space = [Box(low=-10, high=10, shape=(self.share_obs_size,)) for _ in
                                        range(self.n_agents)]

        acdims = [len(ap) for ap in self.agent_partitions] # [2, 2, 2, 2]
        self.action_space = tuple([Box(self.env.action_space.low[sum(acdims[:a]):sum(acdims[:a + 1])], # Box(8,)
                                       self.env.action_space.high[sum(acdims[:a]):sum(acdims[:a + 1])]) for a in
                                   range(self.n_agents)]) # (Box(1,), Box(1,), Box(1,)) / (Box(2,), Box(2,), Box(2,), Box(2,))

        pass

    def step(self, actions):
        # 4, 2
        # 0:array([-0.07234215,  0.3807798 ], dtype=float32) 
        # 1:array([-0.08820119, -0.45039067], dtype=float32)
        # 2:array([0.21782555, 0.02647025], dtype=float32)
        # 3:array([-0.15449826,  0.08340848], dtype=float32)
        # need to remove dummy actions that arise due to unequal action vector sizes across agents
        # [-0.07234215, 0.3807798, -0.08820119, -0.45039067, 0.21782555, 0.026470251, -0.15449826, 0.08340848]
        flat_actions = np.concatenate([actions[i][:self.action_space[i].low.shape[0]] for i in range(self.n_agents)])
        all_last_actions = actions
        obs_n, reward_n, done_n, info_n = self.wrapped_env.step(flat_actions)
        self.steps += 1

        info = {}
        info.update(info_n) # {'reward_forward': -0.04459226965844276, 'reward_ctrl': -0.4381859302520752, 'reward_contact': -0.0, 'reward_survive': 1.0}
        ...
        # list[[35]*4]   list[[115]*4]
        return self.get_obs(), self.get_state(), rewards, dones, infos, self.get_avail_actions(), all_last_actions
    ...
    def reset(self, **kwargs):
        ...
        self.steps = 0
        self.timelimit_env.reset()
        all_last_actions = np.zeros((self.n_agents, self.action_dim), dtype=np.float32)
        return self.get_obs(), self.get_state(), self.get_avail_actions(), all_last_actions

    def render(self, **kwargs):
        # self.env.render(**kwargs)
        self.env.sim.render(mode='window', width=16, height=16, depth=False)
